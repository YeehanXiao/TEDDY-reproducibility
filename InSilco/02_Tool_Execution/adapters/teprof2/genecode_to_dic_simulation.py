...
def set_intron_number(annotation, number):
    """Replace exon_number when present; otherwise append intron_number."""
    for j, field in enumerate(annotation):
        if field.startswith('exon_number '):
            annotation[j] = 'intron_number {}'.format(number)
            return annotation
    annotation.append('intron_number {}'.format(number))
    return annotation
...
# First generate a dictionary for every transcript and its corresonding exons
    for each in genecode:
        ...
        if True:  # simulation compatibility: retain all reference transcripts
    ...
    for transcript in plus[chrom].keys():
        ...
        if n_exons > 1:
            ...
            for  i in range(0,n_exons-1):
                ...
                annotation = set_intron_number(annotation, i+1)
